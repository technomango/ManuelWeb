...
<?php $this->get_admin_screens_header( 'performance' ); ?>
	...
	$run_lighthouse = ! empty( $_GET['lighthouse'] ) || false !== apply_filters( 'awb_lighthouse_api_key', false );

	global $wpdb;
	$results           = (array) $wpdb->get_results( "SELECT COUNT(*) AS posts_count FROM {$wpdb->posts} WHERE post_status = 'publish' OR post_status = 'draft'", ARRAY_A );
	...
	<section class="avada-db-card hidden awb-wizard-section" data-step="2" data-save="0">
		...
		foreach ( $feature_options as $feature_option ) {
			...
			echo $metaboxes->radio_buttonset( $feature_option['id'], $feature_option['label'], $choices, $feature_option['description'], $feature_option['default'], [] );
		}
		?>
		...
		if ( 1 === (int) $settings->get( 'container_legacy_support' ) ) {
			...
			echo '<p class="wizard-recommendation"><i class="fusiona-af-rating"></i> ' . __( 'Recommend disabling. Flex layouts are faster to render and cause less content layout shift.', 'Avada' ) . '</p>';
		}
		?>
		...
		<?php if ( empty( $globals ) || empty( $globals['data']['template_terms']['header'] ) ) : ?>
			<div class="pyre_metabox_field">
				...
				<div class="pyre_field">
					<a class="button button-primary" href="<?php echo admin_url( 'admin.php?page=avada-layouts' ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Build Header', 'Avada' ); ?></a>
				</div>
			</div>
		...
		if ( 'desktop_and_mobile' === $settings->get( 'status_css_animations' ) ) {
			...
			echo '<p class="wizard-recommendation"><i class="fusiona-af-rating"></i> ' . __( 'Recommend setting to desktop only to disable element animations on mobile devices.', 'Avada' ) . '</p>';
		}
		?>
	...
	<section class="avada-db-card hidden awb-wizard-section" data-step="3">
		...
		<?php
		echo $metaboxes->multiple( 'status_fontawesome', $options['status_fontawesome']['label'], $options['status_fontawesome']['choices'], $options['status_fontawesome']['description'] );
		echo $metaboxes->radio_buttonset( 'fontawesome_v4_compatibility', $options['fontawesome_v4_compatibility']['label'], $choices, $options['fontawesome_v4_compatibility']['description'], $options['fontawesome_v4_compatibility']['default'], [] );
		?>
	...
	<section class="avada-db-card hidden awb-wizard-section" data-step="4">
				...
				<div class="pyre_metabox_field fusion-builder-font-family">
					...
					<div class="pyre_field">
						<div class="font-family">
							...
							<select id="<?php echo $family_id; ?>" name="<?php echo $family_id; ?>" class="input-font_family fusion-select-field fusion-skip-init<?php echo ( is_rtl() ) ? ' fusion-select-field-rtl' : ''; ?>" data-default="<?php echo $option['default']['font-family']; ?>" data-value="<?php echo $family_value; ?>" style="width:100%"></select>
						</div>
						<div class="variant fusion-variant-wrapper">
							<div class="fusion-typography-select-wrapper">
								<div class="select_arrow"></div>
								<select id="<?php echo $variant_id; ?>" name="<?php echo $variant_id; ?>" class="input-variant fusion-select-field fusion-skip-init<?php echo ( is_rtl() ) ? ' fusion-select-field-rtl' : ''; ?>" data-default="<?php echo $option['default']['font-weight']; ?>" data-value="<?php echo $variant_value; ?>"  style="width:100%"></select>
							</div>
						</div>
					</div>
				</div>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['font_face_display']['id'], $options['font_face_display']['label'], $options['font_face_display']['choices'], $options['font_face_display']['description'], $options['font_face_display']['default'], [], 'swap-all' );
		echo '<p class="wizard-recommendation"><i class="fusiona-af-rating"></i> ' . __( 'Recommend setting to swap all for the fastest initial load.', 'Avada' ) . '</p>';

		echo $metaboxes->radio_buttonset( $options['gfonts_load_method']['id'], $options['gfonts_load_method']['label'], $options['gfonts_load_method']['choices'], $options['gfonts_load_method']['description'], $options['gfonts_load_method']['default'], [] );
		echo '<p class="wizard-recommendation"><i class="fusiona-af-rating"></i> ' . __( 'Recommend setting to local but CDN will also work.', 'Avada' ) . '</p>';

		echo $metaboxes->radio_buttonset( $options['preload_fonts']['id'], $options['preload_fonts']['label'], $options['preload_fonts']['choices'], $options['preload_fonts']['description'], $options['preload_fonts']['default'], [] );
		echo '<p class="wizard-recommendation"><i class="fusiona-af-rating"></i> ' . __( 'Recommend setting to none unless your largest contentful paint is text or an icon.', 'Avada' ) . '  <a href="https://theme-fusion.com" target="_blank" rel="noopener noreferrer">' . __( 'Find out how to optimize the largest contentful paint (LCP).', 'Avada' ) . '</a></p>';

		echo $metaboxes->multiple( $options['preload_fonts_variants']['id'], $options['preload_fonts_variants']['label'], $options['preload_fonts_variants']['choices'], $options['preload_fonts_variants']['description'] );
		echo $metaboxes->multiple( $options['preload_fonts_subsets']['id'], $options['preload_fonts_subsets']['label'], $options['preload_fonts_subsets']['choices'], $options['preload_fonts_subsets']['description'] );
		?>
	...
	<section class="avada-db-card hidden awb-wizard-section" data-step="6">
		...
		<?php
		echo $metaboxes->range( $options['pw_jpeg_quality']['id'], $options['pw_jpeg_quality']['label'], $options['pw_jpeg_quality']['description'], $options['pw_jpeg_quality']['choices']['min'], $options['pw_jpeg_quality']['choices']['max'], $options['pw_jpeg_quality']['choices']['step'], $options['pw_jpeg_quality']['default'] );
		?>
		...
		<?php
		echo $metaboxes->range( $options['wp_big_image_size_threshold']['id'], $options['wp_big_image_size_threshold']['label'], $options['wp_big_image_size_threshold']['description'], $options['wp_big_image_size_threshold']['choices']['min'], $options['wp_big_image_size_threshold']['choices']['max'], $options['wp_big_image_size_threshold']['choices']['step'], $options['wp_big_image_size_threshold']['default'] );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['lazy_load']['id'], $options['lazy_load']['label'], $options['lazy_load']['choices'], $options['lazy_load']['description'], $options['lazy_load']['default'], [], 'avada' );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['video_facade']['id'], $options['video_facade']['label'], $options['video_facade']['choices'], $options['video_facade']['description'], $options['video_facade']['default'], [] );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['jquery_migrate_disabled']['id'], $options['jquery_migrate_disabled']['label'], $options['jquery_migrate_disabled']['choices'], $options['jquery_migrate_disabled']['description'], $options['jquery_migrate_disabled']['default'], [], 'disabled' );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['js_compiler']['id'], $options['js_compiler']['label'], $on_off, $options['js_compiler']['description'], $options['js_compiler']['default'], [], '1' );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['defer_styles']['id'], $options['defer_styles']['label'], $on_off, $options['defer_styles']['description'], $options['defer_styles']['default'], [], '0' );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['media_queries_async']['id'], $options['media_queries_async']['label'], $on_off, $options['media_queries_async']['description'], $options['media_queries_async']['default'], [], '0' );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['css_cache_method']['id'], $options['css_cache_method']['label'], $options['css_cache_method']['choices'], $options['css_cache_method']['description'], $options['css_cache_method']['default'], [], 'file' );
		?>
		...
		$is_mod_deflate_loaded = is_array( $apache_modules ) && in_array( 'mod_deflate', $apache_modules );

		if ( $is_mod_deflate_loaded ) :
			echo $metaboxes->radio_buttonset( $options['gzip_status']['id'], $options['gzip_status']['label'], $on_off, $options['gzip_status']['description'], $options['gzip_status']['default'], [] );
			?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['defer_jquery']['id'], $options['defer_jquery']['label'], $on_off, $options['defer_jquery']['description'], $options['defer_jquery']['default'], [] );
		?>
		...
		<?php
		echo $metaboxes->radio_buttonset( $options['critical_css']['id'], $options['critical_css']['label'], $on_off, $options['critical_css']['description'], $options['critical_css']['default'], [] );
		?>
			...
			<div class="awb-extra-recommendations">
				...
				<div class="awb-recommendation-holder">
					<div class="awb-extra-recommendation">
						...
						?>
						<h3><?php printf( __( 'Active Plugins: %s', 'Avada' ), $plugin_counter ); ?></h3>
					...
					<div class="awb-extra-recommendation awb-improvements">
						...
						$plugin_counter = '<span class="awb-wizard-count ' . $plugin_state . '">' . $active_plugins . '</span>';
						?>
						<h3><?php printf( __( 'Possible Improvements', 'Avada' ), $plugin_counter ); ?></h3>
