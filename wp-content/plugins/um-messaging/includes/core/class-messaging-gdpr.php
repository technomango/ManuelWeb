...
class Messaging_GDPR {
	...
	function render_download_chat_link( $message_to, $user_id ) {
		...
		<a href="<?php echo esc_url( $this->get_download_url( $response['conversation_id'] ) ) ?>"
		   ...
		   class="um-gdpr-donwload-link um-tip-e" title="<?php esc_attr_e( 'Descargar Historial', 'um-messaging' ); ?>"
		   <?php if ( empty( $response ) || empty( $messages ) ) { ?>style="display: none;"<?php } ?>>
			<i class="fas fa-download chat-icon-clock"></i>
		</a>
	...
	function render_download_all_chats_link() {
		...
		<div class="descarga-chats"><a href="<?php echo esc_url( $this->get_download_url( 'all' ) ) ?>" class="um-link um-gdpr-donwload-link fusion-button fusion-button-default fusion-button-default-size" title="<?php esc_attr_e( 'Descargar Todos los Chats', 'um-messaging' ); ?>"><i class="fas fa-cloud-download-alt chat-icon-download"></i> <?php _e( 'Descargar Chats', 'um-messaging' ) ?></a></div>
		<?php
	}
		...
		if ( $conversation_id == 'all' ) {
			...
			foreach ( $conversations as $conversation ) {
				...
				if ( ! empty( $messages ) ) {
					...
					$file_content .= "=========== Conversación con \"{$from}\" -> \"{$to}\" ===========\r\n\r\n";
