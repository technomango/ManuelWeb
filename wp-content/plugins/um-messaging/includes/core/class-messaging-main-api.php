...
class Messaging_Main_API {
	...
	function human_time_diff( $from, $to = '' ) {
		...
		if ( $diff < 60 ) {
			$since = sprintf( __('hace %s segs','um-messaging'), $diff );
		} elseif ( $diff < HOUR_IN_SECONDS ) {
			$mins = round( $diff / MINUTE_IN_SECONDS );
			if ( $mins <= 1 )
				$mins = 1;
			/* translators: min=minute */
			$since = sprintf( __('hace %s mins','um-messaging'), $mins );
		} elseif ( $diff < DAY_IN_SECONDS && $diff >= HOUR_IN_SECONDS ) {
			$hours = round( $diff / HOUR_IN_SECONDS );
			if ( $hours <= 1 )
				$hours = 1;
			$since = sprintf( __('hace %s horas','um-messaging'), $hours );
		} elseif ( $diff < WEEK_IN_SECONDS && $diff >= DAY_IN_SECONDS ) {
			$days = round( $diff / DAY_IN_SECONDS );
			if ( $days <= 1 )
				$days = 1;
			$since = sprintf( __('hace %s días','um-messaging'), $days );
		} elseif ( $diff < 30 * DAY_IN_SECONDS && $diff >= WEEK_IN_SECONDS ) {
			$weeks = round( $diff / WEEK_IN_SECONDS );
			if ( $weeks <= 1 )
				$weeks = 1;
			$since = sprintf( __('hace %s semanas','um-messaging'), $weeks );
		} elseif ( $diff < YEAR_IN_SECONDS && $diff >= 30 * DAY_IN_SECONDS ) {
			$months = round( $diff / ( 30 * DAY_IN_SECONDS ) );
			if ( $months <= 1 )
				$months = 1;
			$since = sprintf( __('hace %s meses','um-messaging'), $months );
		} elseif ( $diff >= YEAR_IN_SECONDS ) {
			$years = round( $diff / YEAR_IN_SECONDS );
			if ( $years <= 1 )
				$years = 1;
			$since = sprintf( __('hace %s años','um-messaging'), $years );
		}
	...
	function beautiful_time( $time, $pos ) {
		...
		$clean_date_time = date("m-d-Y, h:i A", $from_time );
		...
		if ( $pos == 'right_m' ) {
			return '<i class="glyphicon fa-clock fas um-message-item-time um-tip-e chat-icon-clock" aria-hidden="true" title="" um-messsage-timestamp="' . esc_attr( $from_time ) . '" um-message-utc-time="' . esc_attr( $clean_date_time ) . '">' . '</i>';
		} else {
			return '<i class="glyphicon fa-clock fas um-message-item-time um-tip-w chat-icon-clock" aria-hidden="true" title="" um-messsage-timestamp="' . esc_attr( $from_time ) . '" um-message-utc-time="' . esc_attr( $clean_date_time ) . '">' . '</i>';
		}
	}
