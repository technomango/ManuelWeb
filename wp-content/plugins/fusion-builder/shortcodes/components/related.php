	...
	if ( ! class_exists( 'FusionTB_Related' ) ) {
		...
		class FusionTB_Related extends Fusion_Component {
			...
			public function ajax_query() {
				...
				if ( isset( $_POST['model'] ) && isset( $_POST['model']['params'] ) && ! apply_filters( 'fusion_builder_live_request', false ) ) { // phpcs:ignore WordPress.Security.NonceVerification
					...
					while ( $fusion_query->have_posts() ) :
						...
						ob_start();
						comments_popup_link( __( '0 Comentarios', 'fusion-builder' ), __( '1 Comentario', 'fusion-builder' ), __( '% Comentarios', 'fusion-builder' ) );
			...
			public function render( $args, $content = '' ) {
				...
				$main_heading = esc_html__( 'Artículos Relacionados', 'fusion-builder' );

				if ( 'avada_portfolio' === $this->post_type ) {
					$main_heading = esc_html__( 'Proyectos Relacionados', 'fusion-builder' );
...
function fusion_component_related() {
	...
	fusion_builder_map(
		fusion_builder_frontend_data(
			'FusionTB_Related',
			[
				'name'                    => esc_html__( 'Artículos Relacionados', 'fusion-builder' ),
