		...
		if ( ( $settings['post_meta'] && 'no' !== $post_meta ) || ( ! $settings['post_meta'] && 'yes' === $post_meta ) ) {
			...
			if ( $settings['post_meta_author'] ) {
				...
				if ( $fusion_settings->get( 'disable_date_rich_snippet_pages' ) && $fusion_settings->get( 'disable_rich_snippet_author' ) ) {
					...
					$metadata .= sprintf( esc_html__( 'Por %s', 'fusion-builder' ), '<span class="vcard"><span class="fn">' . $author_post_link . '</span></span>' );
				} else {
					/* translators: The author. */
					$metadata .= sprintf( esc_html__( 'Por %s', 'fusion-builder' ), '<span>' . $author_post_link . '</span>' );
				}
				$metadata .= '<span class="fusion-inline-sep">|</span>';
			...
			if ( $settings['post_meta_cats'] ) {
				...
				if ( $categories ) {
					...
					$metadata .= ( $settings['post_meta_tags'] ) ? sprintf( esc_html__( 'Categorías: %s', 'fusion-builder' ), $categories ) : $categories;
					$metadata .= '<span class="fusion-inline-sep">|</span>';
				}
			}
			...
			if ( $settings['post_meta_comments'] && 'grid_timeline' !== $layout ) {
				if ( 'private' === get_post_status() && ! is_user_logged_in() || in_array( get_post_status(), [ 'pending', 'draft', 'future' ], true ) && ! current_user_can( 'edit-post' ) ) {
					$comments = '<a href="#">' . get_comments_number() . ' ' . esc_html__( 'Comentario(s)', 'fusion-builder' ) . '</a>';
				} else {
					ob_start();
					comments_popup_link( esc_html__( '0 Comentarios', 'fusion-builder' ), esc_html__( '1 Comentario', 'fusion-builder' ), esc_html__( '% Comentario​s', 'fusion-builder' ) );
				...
				<div class="comment-box">
					<div class="comment-author meta">
						...
						printf(
							...
							esc_attr__( '%1$s a las %2$s', 'fusion-builder' ),
						...
						edit_comment_link( __( ' - Editar', 'fusion-builder' ), '  ', '' );

						comment_reply_link(
							array_merge(
								$args,
								[
									'reply_text' => __( ' - Responder', 'fusion-builder' ),
									'add_below'  => 'comment',
									'depth'      => $depth,
					...
					<div class="comment-text">
						<?php if ( '0' == $comment->comment_approved ) : // phpcs:ignore WordPress.PHP.StrictComparisons ?>
							<em><?php esc_attr_e( 'Tu comentario está esperando ser moderado.', 'fusion-builder' ); ?></em>
							<br />
						<?php endif; ?>
						<?php comment_text(); ?>
					</div>
